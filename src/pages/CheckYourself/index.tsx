...
export const CheckYourself: React.FC = () => {
    ...
    useEffect(() => {
        if (point == 10 || point == -5) {
            ...
            // winAudio.play()
            if (point == 10) {
                setStr("congratulations🤗, you answered 10 questions correctly")
            } else {
                setStr("you need to repeat the words🥺")
            }
            setTimeout(() => {
                setShow(false)
            }, 3000)
        ...
        <div className="check">
            <div className="check__wrapper">
                <div className="check__title">
                    ...
                    <br />
                    <p>You need to collect 10 points to win the test, if you collect -5 points you'll fail the test. </p>
                </div>
                ...
                <div className="check__inner-wrapper">
                    ...
                    {
                        show ? <p className="p1">{str}</p> : <></>
                    }
                            {!start ?

                                <div className="check__choices-inps">
                                    <input type="radio" name="choice" id="english" onChange={() => { setChoice(Words.ENGLISH); setBtn(true) }} />
                                    <label htmlFor="english" className="englishLabel">English</label>
                                    <input type="radio" name="choice" id="armenian" onChange={() => { setChoice(Words.ARMENIAN); setBtn(true) }} />
                                    <label htmlFor="armenian" className="armenianLabel">Armenian</label>
                                </div> : <></>}
                            <div className="check__choices-btns">
                                {
                                    btn && !start ?
                                        <button onClick={() => { getSingleWord(); setBtn(false) }} className="accept-btn">Start</button>
